...
func Create(lg *logrus.Logger, dirpath string, metadata []byte) (*WAL, error) {
	if Exist(dirpath) {
	...
	if err := fileutil.CreateDirAll(tmpdirpath); err != nil {
		lg.Warn(
			...
			logrus.WithField("tmp-dir-path", tmpdirpath),
			logrus.WithField("dir-path", dirpath),
	...
	if err = fileutil.Preallocate(f.File, SegmentSizeBytes, true); err != nil {
		lg.Warn(
			...
			logrus.WithField("path", p),
			logrus.WithField("segment-bytes", SegmentSizeBytes),
			logrus.WithError(err),
		)
		return nil, err
	}
	...
	if w, err = w.renameWAL(tmpdirpath); err != nil {
		lg.Warn(
			...
			logrus.WithField("tmp-dir-path", tmpdirpath),
			logrus.WithField("dir-path", logDirPath),
			logrus.WithError(err),
		)
		return nil, err
	}
	...
	if perr != nil {
		lg.Warn(
			...
			logrus.WithField("parent-dir-path", filepath.Dir(w.dir)),
			logrus.WithField("dir-path", w.dir),
			logrus.WithError(perr),
		)
		return nil, perr
	}
...
func (w *WAL) Reopen(lg *logrus.Logger, snap walpb.Snapshot) (*WAL, error) {
	err := w.Close()
	if err != nil {
		lg.Panic("failed to close WAL during reopen", logrus.WithError(err))
	}
...
func (w *WAL) cleanupWAL(lg *logrus.Logger) {
	var err error
	if err = w.Close(); err != nil {
		lg.Panic("failed to close WAL during cleanup", logrus.WithError(err))
	}
	...
	if err = os.Rename(w.dir, brokenDirName); err != nil {
		lg.Panic(
			...
			logrus.WithError(err),
			logrus.WithField("source-path", w.dir),
			logrus.WithField("rename-path", brokenDirName),
		)
	}
}
...
func (w *WAL) renameWALUnlock(tmpdirpath string) (*WAL, error) {
	...
	w.lg.Info(
		...
		logrus.WithField("from", tmpdirpath),
		logrus.WithField("to", w.dir),
	)
	w.Close()
...
// previous records.
func Open(lg *logrus.Logger, dirpath string, snap walpb.Snapshot) (*WAL, error) {
...
func OpenForRead(lg *logrus.Logger, dirpath string, snap walpb.Snapshot) (*WAL, error) {
	return openAtIndex(lg, dirpath, snap, false)
}

func openAtIndex(lg *logrus.Logger, dirpath string, snap walpb.Snapshot, write bool) (*WAL, error) {
	if lg == nil {
		lg = logrus.StandardLogger()
	}
...
func selectWALFiles(lg *logrus.Logger, dirpath string, snap walpb.Snapshot) ([]string, int, error) {
...
func openWALFiles(lg *logrus.Logger, dirpath string, names []string, nameIndex int, write bool) ([]io.Reader, []*fileutil.LockedFile, func() error, error) {
...
func ValidSnapshotEntries(lg *logrus.Logger, walDir string) ([]walpb.Snapshot, error) {
...
func Verify(lg *logrus.Logger, walDir string, snap walpb.Snapshot) (*raftpb.HardState, error) {
	var metadata []byte
	var err error
	var match bool
	var state raftpb.HardState
...
func (w *WAL) cut() error {
	...
	w.lg.Info("created a new WAL segment", logrus.WithField("path", fpath))
	return nil
}
...
func (w *WAL) sync() error {
	...
	if took > warnSyncDuration {
		w.lg.Warn(
			"slow fdatasync",
			logrus.WithField("took", took),
			logrus.WithField("expected-duration", warnSyncDuration),
		)
	}
...
func (w *WAL) Close() error {
	...
	for _, l := range w.locks {
		...
		if err := l.Close(); err != nil {
			w.lg.Error("failed to close WAL", logrus.WithError(err))
		}
	}
...
func (w *WAL) seq() uint64 {
	...
	if err != nil {
		w.lg.Fatal("failed to parse WAL name", logrus.WithField("name", t.Name()), logrus.WithError(err))
	}
	return seq
}

func closeAll(lg *logrus.Logger, rcs ...io.ReadCloser) error {
	stringArr := make([]string, 0)
	for _, f := range rcs {
		if err := f.Close(); err != nil {
			lg.Warn("failed to close: ", logrus.WithError(err))
