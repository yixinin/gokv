...
func TestRepairTruncate(t *testing.T) {
	corruptf := func(p string, offset int64) error {
		f, err := openLast(logrus.StandardLogger(), p)
		if err != nil {
			return err
		}
		defer f.Close()
...
func testRepair(t *testing.T, ents [][]raftpb.Entry, corrupt corruptFunc, expectedEnts int) {
	...
	// create WAL
	w, err := Create(logrus.StandardLogger(), p, nil)
	defer func() {
	...
	w, err = Open(logrus.StandardLogger(), p, walpb.Snapshot{})
	if err != nil {
		t.Fatal(err)
	}
	_, _, _, err = w.ReadAll()
	...
	// repair the wal
	if ok := Repair(logrus.StandardLogger(), p); !ok {
...
func TestRepairFailDeleteDir(t *testing.T) {
	...
	os.RemoveAll(p)
	if Repair(logrus.StandardLogger(), p) {
